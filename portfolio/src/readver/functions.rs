...
pub fn write_csv_data(filename: &str, data1: &[f64], data2: &[f64], data3: &Vec<Vec<f64>>) -> Result<()> {
...
pub fn gen_costs(num_assets: usize) -> Vec<f64> {
...
pub fn gen_covariances(num_assets: usize) -> Vec<Vec<f64>> {
  ...
  for i in 0..num_assets {
      for j in 0..num_assets {
          if i == j {
              cov_matrix[i][j] = rng.gen::<f64>() * 0.50;
          } else {
              cov_matrix[i][j] = rng.gen::<f64>() * 0.50;
          }
      }
